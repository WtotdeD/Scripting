Description
 This PowerShell script displays a simple calculator GUI 

　

 The GUI part of the code is generated by the free program PrimalForms (Community Edition) v1.0.8.0

 The PowerShell code behind the buttons tries to emulate windows calc.exe but there are differences.

The number of decimal places is only 16. (calc.exe has 32)
You can’t use the keyboard to enter (only mouse)
Dividing by zero gives infinity and that can be used in subsequent calculations, (weird but cool).

But on a plus side
The output is logged in the PowerShell console as you work.
The code is editable by you. i.e. easily change the buttons on the right to link to any PowerShell code.
Also the memory is retained after the program is closed.

An interesting project if anything and I found I learnt a few tricks that will help me when I do my next project that needs a GUI.

You can download the PrimalForms project file from here if you want to play with the layout. Though you will find the application a real pain that the code is separate from the project. 
http://painterinfo.com/MiniCalc/MiniCalc-ppf.zip  (right click save as) 
